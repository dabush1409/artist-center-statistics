...
(function () {
    ...
    ExhibitionsController.$inject = ['$scope', 'Constants', '$q', 'Api', 'Maps'];

    function ExhibitionsController($scope, Constants, $q, Api, Maps) {
                ...
                angular.forEach(data.data, function (val) {
                    if (val.peripheryExhibitNotPeriphery > 0) {
                        arr.push({
                            ...
                            count: val.peripheryExhibitNotPeriphery
                        });
                    }
                });
                Maps.initializeMap("map_ex_canvas", arr, 2);
            }, function () {
        ...
        Api.getArtistStatsWowInPeriphery()
            .then(function (data) {
                var arr = [];
                angular.forEach(data.data, function (val) {
                    if (val.notPeripheryExhibitInPeriphery > 0) {
                        arr.push({
                            location: val.location,
                            count: val.notPeripheryExhibitInPeriphery
                        });
                    }
                });
                Maps.initializeMap("map_ex_canvas2", arr, 2);
            }, function () {
